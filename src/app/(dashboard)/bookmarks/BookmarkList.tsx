    ...
    <div>
      ...
      {view === "card" ? (
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {bookmarks.map((bm) => (
            <li key={bm.id} className="rounded-lg border bg-white shadow-sm flex flex-col">
              {bm.ogImage && (
              ...
              <div className="flex flex-1 flex-col gap-1 p-4">
                ...
                <p className="truncate text-xs text-gray-500">{bm.url}</p>
                {bm.memo && <p className="text-sm text-gray-700 line-clamp-3">{bm.memo}</p>}
