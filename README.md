...
- ブックマークの登録・編集・削除（URL・タイトル・メモ・タグ・OGP画像）
- タグによるグルーピング表示（同一ドメインのサブグループ表示・開閉状態の保持）
- キーワード検索
- ドラッグ＆ドロップによる並び替え（ブックマーク・タグ）
- ゴミ箱方式の削除（復元・完全削除）
- API キー認証による外部 REST API（ブックマーク・タグの CRUD／並び替え／ゴミ箱／OGP 取得）
